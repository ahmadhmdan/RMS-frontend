...
const ServiceableItems = () => {
    ...
    return (
        <DataTable
            columns={columns}
            data={items}
            title={t('serviceable_items')}
            onCreate={() => navigate(`${ROUTES.CREATE_ITEM}/serviceable`)}
            onEdit={(item) => navigate(`${ROUTES.EDIT_ITEM}/${item.id}/serviceable`)}
            showEdit={true}
            onDelete={deleteItem}
            showView={false}
...
export default ServiceableItems;