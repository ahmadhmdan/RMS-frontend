...
import { getSuppliers, getInventories, getCurrencies, getUnits } from '../../invoices/services/createInvoices.service';
...
export const useInvoiceData = () => {
    ...
    useEffect(() => {
        if (invoiceQuery.data) {
            setSuppliers(invoiceQuery.data.supRes);
    ...
    useEffect(() => {
        if (itemQuery.data) {
            const filteredItems = itemQuery.data.filter((i: any) => i.type === 1);
            setItemsList(filteredItems);
            setItemsMap(filteredItems.reduce((acc: any, it: any) => ({ ...acc, [it.id]: it }), {}));
        }
    }, [itemQuery.data]);
