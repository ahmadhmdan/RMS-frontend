...
const ReverseProducedItems = () => {
    const { t } = useTranslation();
    const navigate = useNavigate()
    const { items, loading, page, total, pageSize, setPage, categories, itemGroups, search, setSearch, categoryId, setCategoryId, itemGroupId, setItemGroupId, deleteItem, setPageSize } = useItems();
    ...
    return (
        <DataTable
            columns={columns}
            data={items}
            title={t('reverse_produced_items')}
            onCreate={() => navigate(`${ROUTES.CREATE_ITEM}/reverse-produced`)}
            onEdit={(item) => navigate(`${ROUTES.EDIT_ITEM}/${item.id}/reverse-produced`)}
            showEdit={true}
            onDelete={deleteItem}
            showView={false}
...
export default ReverseProducedItems;