...
import { useInvoiceData } from '../hooks/useCreateGoods';
import { getInvoiceNumber, createInvoice } from '../services/goods-invoices.service'
import '../components/CreateInvoice_3.css';
...
type GoodsItem = InvoiceLineItemBase;

type FormValues = InventoryInvoiceFormValues<GoodsItem>;

const CreateGoodsInvoice: React.FC = () => {
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                Swal.fire({
                    ...
                    navigate(ROUTES.GOODS_INVOICE);
                });
            } catch (error: any) {
                console.error('Error creating goods invoice:', error);
                Swal.fire({
                    icon: 'error',
                    title: t('error'),
    ...
    const handleKeyDown = (
        ...
        rowIndex: number,
        fieldIndex: number,
        arrayHelpers: { push: (obj: GoodsItem) => void; remove: (index: number) => void }
    ) => {
        ...
        <div className="card">
            <div className="card-header border-0 pt-6">
                <div className="card-title d-flex align-items-center gap-4">
                    <h2 className="fw-bold fs-2 mb-0">{t('goods_invoice')}</h2>
                ...
                <FormikProvider value={formik}>
                    <form onSubmit={formik.handleSubmit}>
                        ...
                        <div className="table-responsive">
                            <table className={`table table-bordered excel-like-table-3 ${mode === 'dark' ? 'table-dark-mode' : ''}`}>
                                <thead>
                                        ...
                                        <>
                                            <tbody>
                                                {values.items.map((item, index) => {
                                                    ...
                                                    const itemErrors = Array.isArray(errors.items) ? errors.items[index] as FormikErrors<GoodsItem> : undefined;
                        ...
                        <div className="row justify-content-end mt-8">
                            <div className="col-lg-6 col-xl-6">
                                <div className="bg-light rounded-4 p-6 shadow-sm">
                                    <h3 className="fw-bold text-dark mb-6">{t("invoice_summary")}</h3>
...
export default CreateGoodsInvoice;