...
import { useTransferInvoice } from '../hooks/useTransferInvoice'
...
const TransferInvoice = () => {
    ...
    const columns = [
        ...
        { key: 'from_inventory_name', header: t('from_inventory_warehouse') },
        { key: 'inventory_name', header: t('to_inventory_warehouse') },
        {
            key: 'invoice_date',
    ...
    const handleView = (invoice: any) => {
        navigate(`${ROUTES.TRANSFER_INVOICE_DETAILS}/${invoice.id}`)
    }
    ...
    return (
        <DataTable
            columns={columns}
            data={invoices}
            title={t('transfer_invoices')}
            onCreate={() => navigate(ROUTES.CREATE_TRANSFER_INVOICE)}
            onEdit={() => { }}
            showEdit={false}
