...
import { useWasteInvoiceData } from '../hooks/useCreateWaste';
import { getWasteInvoiceNumber, createWasteInvoice } from '../services/waste-invoices.service'
...
const CreateWasteInvoice: React.FC = () => {
    ...
    const { mode } = useTheme();
    const { t } = useTranslation();
    const { inventories, itemsList, unitsMap, fetchUnits, loading } = useWasteInvoiceData();

    useEffect(() => {
        const fetchInvoiceNo = async () => {
            try {
                const res = await getWasteInvoiceNumber();
                if (res.success) {
    ...
    const formik = useFormik<FormValues>({
        ...
        onSubmit: async (values) => {
            ...
            try {
                await createWasteInvoice(payload);
                Swal.fire({
                    icon: 'success',
                    title: t('success'),
                    ...
                    navigate(ROUTES.WASTE_INVOICE);
                });
            } catch (error: any) {
                console.error('Error creating waste invoice:', error);
                Swal.fire({
                    icon: 'error',
                    title: t('error'),
    ...
    const handleKeyDown = (
        ...
        rowIndex: number,
        fieldIndex: number,
        arrayHelpers: { push: (obj: WasteItem) => void; remove: (index: number) => void }
    ) => {
        ...
        <div className="card">
            <div className="card-header border-0 pt-6">
                <div className="card-title d-flex align-items-center gap-4">
                    <h2 className="fw-bold fs-2 mb-0">{t('waste_invoice')}</h2>
                                        ...
                                        <>
                                            <tbody>
                                                {values.items.map((item, index) => {
                                                    ...
                                                    const itemErrors = Array.isArray(errors.items) ? errors.items[index] as FormikErrors<WasteItem> : undefined;
                        ...
                        <div className="row justify-content-end mt-8">
                            <div className="col-lg-6 col-xl-6">
                                <div className="bg-light rounded-4 p-6 shadow-sm">
                                    ...
                                    <div className="d-flex justify-content-between align-items-center pt-5 border-top border-4 border-danger">
                                        <h3 className="mb-0 fw-bolder text-dark">{t("total")}</h3>
                                        <h2 className="mb-0 fw-bolder text-danger">
                                            {formatNumber(values.items.reduce((s, i) => s + (Number(i.quantity) || 0), 0).toFixed(2))}
...
export default CreateWasteInvoice;