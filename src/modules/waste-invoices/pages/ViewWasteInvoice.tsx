...
import { wasteInvoicesService } from '../services/waste-invoices.service'
...
const ViewWasteInvoice = () => {
    ...
    useEffect(() => {
        const fetchInvoice = async () => {
            if (!id) return
            try {
                const res = await wasteInvoicesService.getById(Number(id))
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, t('waste_invoice'));
        XLSX.writeFile(workbook, `Waste_Invoice_${invoice.invoice_no}.xlsx`);
    };
        ...
        <div className="card">
            <div className="card-header border-0 pt-8 pb-6">
                <div className="card-title">
                    <h2 className="fw-bolder">
                        {t('waste_invoice')} #{invoice.invoice_no}
                    </h2>
                </div>
            ...
            <div className="card-body p-10">
                <div className="row g-8 mb-10">
                    ...
                    <div className="col-lg-6">
                        <div className="bg-light-danger rounded-3 p-6 h-100">
                            <div className="text-center">
                                <h4 className="fw-bold text-danger mb-8">{t('total')}</h4>
                                <div className="d-flex align-items-baseline justify-content-center gap-2">
                                    <span className="fs-2x fw-bolder text-danger">{totalQuantity}</span>
                                    <span className="text-gray-600 fs-4 fw-medium">{t('units')}</span>
                                </div>
                                <div className="mt-6">
                                    <h5 className="fw-bold text-danger mb-2">{t('total_cost')}</h5>
                                    <div className="fs-2x fw-bolder text-danger">
                        ...
                        {invoice.invoice_details.length > 0 && (
                            <tfoot>
                                <tr className="bg-light fw-bold">
                                    ...
                                    <td className="text-center fs-5 text-danger p-3">{totalQuantity}</td>
                                    <td colSpan={1}></td>
                                    <td className="text-center fs-5 text-danger">{formatCurrency(invoice.total_price, fallbackSymbol)}</td>
                                    <td colSpan={1}></td>
                                </tr>
                            </tfoot>
                        )}
...
export default ViewWasteInvoice