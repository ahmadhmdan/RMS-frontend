...
const HalfProducedItems = () => {
    ...
    return (
        <DataTable
            columns={columns}
            data={items}
            title={t('half_produced_items')}
            onCreate={() => navigate(`${ROUTES.CREATE_ITEM}/half-produced`)}
            onEdit={(item) => navigate(`${ROUTES.EDIT_ITEM}/${item.id}/half-produced`)}
            showEdit={true}
            showView={false}
...
export default HalfProducedItems;