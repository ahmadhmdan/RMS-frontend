...
import { useInvoiceData } from '../hooks/useCreateInvoices';
import { createInvoice, getInvoiceNumber } from '../services/createInvoices.service';
...
import type { DiscountInvoiceFormValues, InvoiceLineItemBase } from '../../../core/types/invoice';
import { calculateDiscountAmounts, calculateInvoiceSubtotal } from '../../../core/utils/invoiceCalculations';

type InvoiceItem = InvoiceLineItemBase & {
    unit_price: number;
    expiration_date?: string;
};

type FormValues = DiscountInvoiceFormValues<InvoiceItem>;
...
const PurchaseInvoice: React.FC = () => {
    ...
    const { inventories, itemsList, itemsMap, currencies, unitsMap, fetchUnits, loading } = useInvoiceData();
    const [filters,] = useState<{ search_name: string; type: string }>({ search_name: '', type: '' })
    const { suppliers, createSupplier } = useSuppliers(filters);
    const payMethods = ['Cash', 'Credit', 'Bank Transfer'];
    ...
    const validationSchema = Yup.object().shape({
        ...
        items: Yup.array().of(
            Yup.object().shape({
                ...
                // unit_price: Yup.number().positive(t('price_must_be_positive')).optional(),
            })
    ...
    const formik = useFormik<FormValues>({
        initialValues: {
            ...
            discountPercent: 0,
            discountAmount: 0,
            items: [{ item_id: '', unit_id: '', expiration_date: '', quantity: 0, unit_price: 0 }],
            pay_method: 'Cash',
            description: '',
            currency_id: '',
            exchange: 1,
        ...
        onSubmit: async (values) => {
            const subtotal = calculateInvoiceSubtotal(values.items);
            ...
            try {
                ...
                Swal.fire({
                    ...
                    navigate(ROUTES.PURCHASE_INVOICES);
                });
            } catch (error: any) {
    ...
    const handleItemChange = (id: string, index: number) => {
        ...
        if (id) {
            ...
            if (item) {
                formik.setFieldValue(`items.${index}.unit_price`, item.last_price || 0);
            }
        }
    };
    ...
    const calculateSubtotal = () => calculateInvoiceSubtotal(values.items);
    const handleDiscountPercentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        formik.setFieldValue('discountAmount', discount.discountAmount);
    };
    const handleDiscountAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        formik.setFieldValue('discountPercent', discount.discountPercent);
    };
    useEffect(() => {
        const subtotal = calculateSubtotal();
        const discount = calculateDiscountAmounts({
    ...
    const handleKeyDown = (
        ...
        // Handle Enter and Tab keys - both should navigate fields and create new rows
        if (e.key === 'Enter' || e.key === 'Tab') {
            if (fieldIndex < fieldsPerRow - 1) {
                ...
                arrayHelpers.push({ item_id: '', unit_id: '', expiration_date: '', quantity: 0, unit_price: 0 });
            }
            return;
        }
    ...
    const filteredSuppliers = suppliers.filter((sup: any) =>
        sup.type === SUPPLIER_TYPE.SUPPLIER || sup.type === SUPPLIER_TYPE.BOTH
    );
            ...
            <div className="card" >
                <div className="card-header border-0 pt-6" >
                    <div className="card-title d-flex align-items-center gap-4" >
                        <h2 className="fw-bold fs-2 mb-0" > {t("purchase_invoice")} </h2>
                        ...
                        <form onSubmit={formik.handleSubmit}>
                            <div className="row mb-7 g-4" >
                                <div className="col-md-4 fv-row" >
                                    <label className="fs-6 fw-semibold mb-2" > {t("supplier")} </label>
                                ...
                                <table className={`table table-bordered excel-like-table ${mode === 'dark' ? 'table-dark-mode' : ''}`}>
                                    <thead>
                                        <tr style={{ backgroundColor: mode === 'light' ? '#f9f9f9' : '#15171c' }}>
                                            ...
                                            <th className="text-center" style={{ width: '15%' }}> {t("expiry_date")} </th>
                                    ...
                                    <FieldArray name="items" >
                                        {({ push, remove }) => (
                                            <>
                                                <tbody>
                                                    {
                                                        values.items.map((item, index) => {
                                                            ...
                                                            const hasExpirationError = touched.items?.[index]?.expiration_date && !!itemErrors?.expiration_date;
                                                            ...
                                                            return (
                                                                <tr key={index} >
                                                                    ...
                                                                    <td >
                                                                        <input
                                                                            type="date"
                                                                            className={`form-control ${hasExpirationError ? 'is-invalid' : ''}`}
                                                                            name={`items.${index}.expiration_date`}
                                                                            value={item.expiration_date}
                                                                            onChange={formik.handleChange}
                                                                            onKeyDown={(e) => handleKeyDown(e, index, 4, { push, remove })}
                                                                            ref={(el) => { rowRefs.current[index][4] = el }}
                                                                        />
                                                                    </td>
                                                                    <td >
                                                                        <input
                                                ...
                                                <tfoot >
                                                    <tr>
                                                        <td colSpan={8} className="p-3 text-start" >
                                                            <button
                                                                type="button"
                                                                className="btn btn-light btn-sm"
                                                                onClick={() =>
                                                                    push({ item_id: '', unit_id: '', expiration_date: '', quantity: 0, unit_price: 0 })
                                                                }
                                                            >
                                                                <i className="ki-outline ki-plus fs-2" style={{ paddingLeft: '0px' }} ></i>
