...
import { useConsumptionInvoiceData } from '../hooks/useCreateConsumption';
import { getConsumptionInvoiceNumber, createConsumptionInvoice } from '../services/consumption-invoices.service'
    ...
    return (
        <div className="position-relative" ref={dropdownRef}>
            ...
            {isOpen && filteredItems.length > 0 && (
                <ul className="list-group position-absolute w-100" style={{ zIndex: 10, maxHeight: '200px', overflowY: 'auto', cursor: 'pointer' }}>
...
const CreateConsumptionInvoice: React.FC = () => {
    ...
    const { mode } = useTheme();
    const { t } = useTranslation();
    const { inventories, itemsList, unitsMap, fetchUnits, loading } = useConsumptionInvoiceData();

    useEffect(() => {
        const fetchInvoiceNo = async () => {
            try {
                const res = await getConsumptionInvoiceNumber();
                if (res.success) {
    ...
    const formik = useFormik<FormValues>({
        ...
        onSubmit: async (values) => {
            ...
            try {
                await createConsumptionInvoice(payload);
                Swal.fire({
                    icon: 'success',
                    title: t('success'),
                    ...
                    navigate(ROUTES.CONSUMPTION_INVOICE);
                });
            } catch (error: any) {
                console.error('Error creating consumption invoice:', error);
                Swal.fire({
                    icon: 'error',
                    title: t('error'),
    ...
    const handleKeyDown = (
        ...
        rowIndex: number,
        fieldIndex: number,
        arrayHelpers: { push: (obj: ConsumptionItem) => void; remove: (index: number) => void }
    ) => {
        ...
        <div className="card">
            <div className="card-header border-0 pt-6">
                <div className="card-title d-flex align-items-center gap-4">
                    <h2 className="fw-bold fs-2 mb-0">{t('consumption_invoice')}</h2>
                    ...
                    <form onSubmit={formik.handleSubmit}>
                        <div className="row mb-7 g-4">
                            <div className="col-md-4 fv-row">
                                ...
                                <select className="form-select form-select-solid" name="inventory" value={values.inventory} onChange={formik.handleChange} required>
                            ...
                            <div className="col-md-4 fv-row">
                                ...
                                <input type="date" className="form-control form-control-solid" name="invoiceDate" value={values.invoiceDate} onChange={formik.handleChange} />
                        ...
                        <div className="row mb-7">
                            <div className="col-12 fv-row">
                                ...
                                <textarea className="form-control form-control-solid" name="description" value={values.description} onChange={formik.handleChange} rows={2} />
                            </div>
                        </div>
                        ...
                        <div className="table-responsive">
                                        ...
                                        <>
                                            <tbody>
                                                {values.items.map((item, index) => {
                                                    ...
                                                    const itemErrors = Array.isArray(errors.items) ? errors.items[index] as FormikErrors<ConsumptionItem> : undefined;
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                <ItemAutocomplete
                                                                    ...
                                                                    inputRef={(el) => { rowRefs.current[index][0] = el }}
                                            ...
                                            <tfoot>
                                                <tr>
                                                    <td colSpan={5} className="p-3 text-start">
                                                        <button type="button" className="btn btn-light btn-sm" onClick={() => push({ item_id: '', unit_id: '', quantity: 0 })}>
                            ...
                            <ErrorMessage name="items">
                                {(msg) => typeof msg === 'string' && <div className="text-danger fs-6 mt-1">{msg}</div>}
                            </ErrorMessage>
                        </div>
                        <div className="separator separator-dashed my-8"></div>
                        <div className="row justify-content-end mt-8">
                            <div className="col-lg-6 col-xl-6">
                                <div className="bg-light rounded-4 p-6 shadow-sm">
                                    <h3 className="fw-bold text-dark mb-6">{t("summary")}</h3>
                                    <div className="d-flex justify-content-between align-items-center mb-5">
                                        <span className="fs-4 fw-semibold text-gray-700">{t("total_items")}</span>
                                        <span className="fs-3 fw-bold text-dark">
                                            {formatNumber(values.items.filter(i => i.item_id && i.quantity > 0).length)}
                                        </span>
                                    </div>
                                    <div className="d-flex justify-content-between align-items-center mb-7">
                                        <span className="fs-4 fw-semibold text-gray-700">{t("total_quantity")}</span>
                                        <span className="fs-3 fw-bold text-dark">
                                            {formatNumber(values.items.reduce((s, i) => s + (Number(i.quantity) || 0), 0).toFixed(2))}
                                        </span>
                                    </div>
                                    <div className="d-flex justify-content-between align-items-center pt-5 border-top border-4 border-warning">
...
export default CreateConsumptionInvoice;